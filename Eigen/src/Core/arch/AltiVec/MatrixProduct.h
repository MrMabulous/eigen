...
struct symm_pack_rhs<std::complex<float>, Index, nr, StorageOrder>
{
  void operator()(std::complex<float>* blockB, const std::complex<float>* _rhs, Index rhsStride, Index rows, Index cols, Index k2)
  {
    ...
    const_blas_data_mapper<std::complex<float>, Index, StorageOrder> rhs(_rhs, rhsStride);
    const int vectorSize = quad_traits<float>::vectorsize;
    float* blockBf = reinterpret_cast<float *>(blockB);
    ...
    for(; j + vectorSize < cols; j+=vectorSize)
    {
        ...
        for(; i < depth; i++)
        {
          ...
          {
            std::complex<float> v = getAdjointVal<float, Index, StorageOrder>(i, j + k, rhs);
            blockBf[ri + k] = v.real();
          }
          ri += vectorSize;
        }
    ...
    for(Index i = k2; i < depth; i++)
    {
        ...
        for(; k < cols; k++)
        {
          std::complex<float> v = getAdjointVal<float, Index, StorageOrder>(i, k, rhs);
          blockBf[ri] = v.real();
          ri += 1;
        }
    }
...
struct symm_pack_lhs<std::complex<float>, Index, Pack1, Pack2_dummy, StorageOrder>
{
  void operator()(std::complex<float>* blockA, const std::complex<float>* _lhs, Index lhsStride, Index cols, Index rows)
  {
    ...
    const_blas_data_mapper<std::complex<float>, Index, StorageOrder> lhs(_lhs, lhsStride);
    const int vectorSize = quad_traits<float>::vectorsize;
    Index ri = 0, j = 0;
    float *blockAf = (float *)(blockA);
    ...
    for(; j + vectorSize < rows; j+=vectorSize)
    {
        ...
        for(; i < depth; i++)
        {
          ...
          {
            std::complex<float> v = getAdjointVal<float, Index, StorageOrder>(j+k, i, lhs);
            blockAf[ri + k] = v.real();
          }
          ri += vectorSize;
        }
    ...
    for(Index i = 0; i < depth; i++)
    {
        ...
        for(; k < rows; k++)
        {
            std::complex<float> v = getAdjointVal<float, Index, StorageOrder>(k, i, lhs);
            blockAf[ri] = v.real();
            ri += 1;
        }
    }
...
struct symm_pack_rhs<std::complex<double>, Index, nr, StorageOrder>
{
  ...
  {
    const Index depth = k2 + rows;
    const_blas_data_mapper<std::complex<double>, Index, StorageOrder> rhs(_rhs, rhsStride);
    const int vectorSize = 2*quad_traits<double>::vectorsize;
    double* blockBf = reinterpret_cast<double *>(blockB);
    ...
    for(; j + vectorSize < cols; j+=vectorSize)
    {
        ...
        for(; i < depth; i++)
        {
          for(Index k = 0; k < vectorSize; k++)
          {
            std::complex<double> v = getAdjointVal<double, Index, StorageOrder>(i, j + k, rhs);
            blockBf[ri + k] = v.real();
          }
          ri += vectorSize;
        }
    ...
    for(Index i = k2; i < depth; i++)
    {
        ...
        for(; k < cols; k++)
        {
          std::complex<double> v = getAdjointVal<double, Index, StorageOrder>(i, k, rhs);
          blockBf[ri] = v.real();
          ri += 1;
        }
    }
...
struct symm_pack_lhs<std::complex<double>, Index, Pack1, Pack2_dummy, StorageOrder>
{
  ...
  {
    const Index depth = cols;
    const_blas_data_mapper<std::complex<double>, Index, StorageOrder> lhs(_lhs, lhsStride);
    const int vectorSize = quad_traits<double>::vectorsize;
    Index ri = 0, j = 0;
    double *blockAf = (double *)(blockA);
    ...
    for(; j + vectorSize < rows; j+=vectorSize)
    {
        ...
        for(; i < depth; i++)
        {
          for(int k = 0; k < vectorSize; k++)
          {
            std::complex<double> v = getAdjointVal<double, Index, StorageOrder>(j+k, i, lhs);
            blockAf[ri + k] = v.real();
          }
          ri += vectorSize;
        }
    ...
    for(Index i = 0; i < depth; i++)
    {
        ...
        for(; k < rows; k++)
        {
            std::complex<double> v = getAdjointVal<double, Index, StorageOrder>(k, i, lhs);
            blockAf[ri] = v.real();
            ri += 1;
        }
    }
...
struct symm_pack_rhs<float, Index, nr, StorageOrder>
{
  ...
  {
    const Index depth = k2 + rows;
    const_blas_data_mapper<float, Index, StorageOrder> rhs(_rhs, rhsStride);
    const int vectorSize = quad_traits<float>::vectorsize;
    Index ri = 0, j = 0;
    for(; j + vectorSize < cols; j+=vectorSize)
    {
...
struct symm_pack_lhs<float, Index, Pack1, Pack2_dummy, StorageOrder>
{
  ...
  {
    const Index depth = cols;
    const_blas_data_mapper<float, Index, StorageOrder> lhs(_lhs, lhsStride);
    const int vectorSize = quad_traits<float>::vectorsize;
    Index ri = 0, j = 0;
...
struct symm_pack_rhs<double, Index, nr, StorageOrder>
{
  ...
  {
    const Index depth = k2 + rows;
    const_blas_data_mapper<double, Index, StorageOrder> rhs(_rhs, rhsStride);
    const int vectorSize = quad_traits<double>::vectorsize;
    Index ri = 0, j = 0;
...
struct symm_pack_lhs<double, Index, Pack1, Pack2_dummy, StorageOrder>
{
  ...
  {
    const Index depth = cols;
    const_blas_data_mapper<double, Index, StorageOrder> lhs(_lhs, lhsStride);
    const int vectorSize = quad_traits<double>::vectorsize;
    Index ri = 0, j = 0;
  ...
  EIGEN_STRONG_INLINE void operator()(std::complex<Scalar>* blockA, const DataMapper& lhs, Index depth, Index rows, Index stride, Index offset)
  {
    ...
    for(j = 0; j + vectorSize < rows; j+=vectorSize)
    {
      ...
      for(; i + vectorSize < depth; i+=vectorSize)
      {
        ...
        pstore<Scalar>((Scalar *)(blockAt + ri     ), block.packet[0]);
        pstore<Scalar>((Scalar *)(blockAt + ri +  4), block.packet[1]);
        pstore<Scalar>((Scalar *)(blockAt + ri +  8), block.packet[2]);
        pstore<Scalar>((Scalar *)(blockAt + ri + 12), block.packet[3]);
  ...
  EIGEN_STRONG_INLINE void operator()(Scalar* blockA, const DataMapper& lhs, Index depth, Index rows, Index stride, Index offset)
  {
    ...
    for(j = 0; j + vectorSize < rows; j+=vectorSize)
    {
      ...
      for(; i + vectorSize < depth; i+=vectorSize)
      {
        ...
        pstore<Scalar>((Scalar *)(blockA + ri     ), block.packet[0]);
        pstore<Scalar>((Scalar *)(blockA + ri +  4), block.packet[1]);
        pstore<Scalar>((Scalar *)(blockA + ri +  8), block.packet[2]);
        pstore<Scalar>((Scalar *)(blockA + ri + 12), block.packet[3]);
      ...
      for(; i < depth; i++)
      {
        if(StorageOrder == RowMajor)
        {
          ...
          pstore<Scalar>((Scalar *)(blockA + ri), lhsV);
        }
  ...
  EIGEN_STRONG_INLINE void operator()(std::complex<Scalar>* blockB, const DataMapper& rhs, Index depth, Index cols, Index stride, Index offset)
  {
    ...
    for(; j + vectorSize < cols; j+=vectorSize)
    {
        ...
        for(; i + vectorSize < depth; i+=vectorSize)
        {
            ...
            pstore<Scalar>((Scalar *)(blockBt + ri     ), block.packet[0]);
            pstore<Scalar>((Scalar *)(blockBt + ri +  4), block.packet[1]);
            pstore<Scalar>((Scalar *)(blockBt + ri +  8), block.packet[2]);
            pstore<Scalar>((Scalar *)(blockBt + ri + 12), block.packet[3]);
  ...
  EIGEN_STRONG_INLINE void operator()(Scalar* blockB, const DataMapper& rhs, Index depth, Index cols, Index stride, Index offset)
  {
    ...
    for(; j + vectorSize < cols; j+=vectorSize)
    {
      ...
      for(; i + vectorSize < depth; i+=vectorSize)
      {
        ...
        pstore<Scalar>((Scalar *)(blockB + ri     ), block.packet[0]);
        pstore<Scalar>((Scalar *)(blockB + ri +  4), block.packet[1]);
        pstore<Scalar>((Scalar *)(blockB + ri +  8), block.packet[2]);
        pstore<Scalar>((Scalar *)(blockB + ri + 12), block.packet[3]);
      ...
      for(; i < depth; i++)
      {
        if(StorageOrder == ColMajor)
        {
          ...
          pstore<Scalar>((Scalar *)(blockB + ri), rhsV);
        }
        ri += vectorSize;
      }
  ...
  EIGEN_STRONG_INLINE void operator()(double* blockA, const DataMapper& lhs, Index depth, Index rows, Index stride, Index offset)
  {
    ...
    for(j = 0; j + vectorSize < rows; j+=vectorSize)
    {
      ...
      for(; i + vectorSize < depth; i+=vectorSize)
      {
        ...
        pstore<double>((double *)(blockA + ri    ), block.packet[0]);
        pstore<double>((double *)(blockA + ri + 2), block.packet[1]);
      ...
      for(; i < depth; i++)
      {
        if(StorageOrder == RowMajor)
        {
          ...
          pstore<double>((double *)(blockA + ri), lhsV);
        }
...
// [a3 a4 b3 b4]
          ...
          pstore<double>((double *)(blockB + ri    ), block1.packet[0]);
          pstore<double>((double *)(blockB + ri + 2), block2.packet[0]);
          pstore<double>((double *)(blockB + ri + 4), block1.packet[1]);
          pstore<double>((double *)(blockB + ri + 6), block2.packet[1]);
        } else {
          ...
          pstore<double>((double *)(blockB + ri    ), block.packet[0]);
          pstore<double>((double *)(blockB + ri + 2), block.packet[1]);
          pstore<double>((double *)(blockB + ri + 4), block.packet[2]);
          pstore<double>((double *)(blockB + ri + 6), block.packet[3]);
        }
      ...
      for(; i < depth; i++)
      {
        if(StorageOrder == ColMajor)
        {
          ...
          pstore<double>((double *)(blockB + ri), rhsV);
  ...
  EIGEN_STRONG_INLINE void operator()(std::complex<double>* blockA, const DataMapper& lhs, Index depth, Index rows, Index stride, Index offset)
  {
    ...
    for(j = 0; j + vectorSize < rows; j+=vectorSize)
    {
      ...
      for(; i + vectorSize < depth; i+=vectorSize)
      {
        ...
        pstore<double>((double *)(blockAt + ri     ), block.packet[0]);
        pstore<double>((double *)(blockAt + ri +  2), block.packet[1]);
      ...
      for(; i + vectorSize < depth; i+=vectorSize)
      {
        ...
        if(StorageOrder == ColMajor)
        {
          ...
          block.packet[0] = vec_perm(cblock.packet[0].v , cblock.packet[2].v, p16uc_GETIMAG64); //[a1i a2i]
          block.packet[1] = vec_perm(cblock.packet[1].v , cblock.packet[3].v, p16uc_GETIMAG64); //[b1i b2i]
        } else {
          cblock.packet[0] = lhs.template loadPacket<PacketC>(j + 0, i); //[a1 a1i]
          cblock.packet[1] = lhs.template loadPacket<PacketC>(j + 1, i); //[a2 a2i]
          ...
          block.packet[0] = vec_perm(cblock.packet[0].v , cblock.packet[1].v, p16uc_GETIMAG64); //[a1i a2i]
          block.packet[1] = vec_perm(cblock.packet[2].v , cblock.packet[3].v, p16uc_GETIMAG64); //[b1i b2i]
        }
  ...
  EIGEN_STRONG_INLINE void operator()(std::complex<double>* blockB, const DataMapper& rhs, Index depth, Index cols, Index stride, Index offset)
  {
    ...
    for(; j + 2*vectorSize < cols; j+=2*vectorSize)
    {
      ...
      for(; i < depth; i++)
      {
        ...
        pstore<double>((double *)(blockBt + ri    ), block.packet[0]);
        pstore<double>((double *)(blockBt + ri + 2), block.packet[1]);
