...
// same output address. It basically reads the current value of the output
// variable, and attempts to update it with the new value. If in the meantime
// another gpu thread updated the content of the output address it will try
// again.
...
#pragma unroll
  for (int offset = warpSize / 2; offset > 0; offset /= 2) {
    ...
    // __shfl_down is ambguous" error and list the float and int versions of
    // __shfl_down as the candidate functions.
...
template <int BlockSize, int NumPerThread, typename Self, typename Reducer,
          typename Index>
__global__ void FullReductionKernelHalfFloat(
...
#pragma unroll
  for (int offset = warpSize / 2; offset > 0; offset /= 2) {
#if defined(EIGEN_HIPCC)
    // FIXME : remove this workaround once we have native half/half2 support for
    // __shfl_down
    union {
      int i;
      half2 h;
...
__global__ void ReductionCleanupKernelHalfFloat(
...
#pragma unroll
      for (int offset = warpSize / 2; offset > 0; offset /= 2) {
        ...
        // to __shfl_down is ambguous" error and list the float and int versions
        // of __shfl_down as the candidate functions.
